...
void EasyMotionTarget::nextGroup()
{
    m_currentGroup++;
...
void EasyMotionTarget::previousGroup()
{
    m_currentGroup--;
    if (m_currentGroup  < 0) {
...
int EasyMotionTarget::getFirstTargetIndex() const { return m_currentGroup * GroupSize;  }
...
int EasyMotionTarget::getGroupNum()
{
...
void EasyMotionTarget::initCode()
{
    m_code.reserve(62);
    for (int i = 0; i < 26; ++i) {
        m_code << QChar(i + 'a');
